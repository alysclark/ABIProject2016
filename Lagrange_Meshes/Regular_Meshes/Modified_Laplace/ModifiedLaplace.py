...
mesh2.NumberOfComponentsSet(number_of_mesh_components)
mesh2.NumberOfElementsSet(total_number_of_elements)
...
for elem_num in range (1,total_number_of_elements+1):
    element_nodes = originalElements.NodesGet(elem_num,nodes_per_elem)
    #print 'original node numbers: ', element_nodes
    elements.NodesSet(elem_num,element_nodes)
    selected_element_nodes += element_nodes.tolist()
    #print 'all new node numbers: ', selected_element_nodes
...
geometricField1.LabelSet('geometricField1')
...
geometricField2.LabelSet('geometricField2')
...
fml.OutputAddFieldNoType(baseName+".phi", dataFormat, dependentField,
    iron.FieldVariableTypes.U, iron.FieldParameterSetTypes.VALUES)
fml.OutputWrite("ModifiedLaplace.xml")
fml.Finalise()

# Export results
fields = iron.Fields()
fields.CreateRegion(region)
fields.NodesExport("ModifiedLaplaceResults","FORTRAN")
fields.ElementsExport("ModifiedLaplaceResults","FORTRAN")
fields.Finalise()
