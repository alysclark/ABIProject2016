...
removed_elements = [1, 5, 8, 11, 12, 15, 16, 17, 18, 21, 25, 29, 31, 167, 168, 169, 170, 171, 172, 173, 175, 177, 178, 180, 182, 188, 190, 193, 195, 197, 204, 206, 208, 211, 750]
unique_elements = dict.fromkeys(removed_elements).keys()
ordered_elements = (numpy.array(sorted(unique_elements)) - 1).tolist() # list of elements to be removed from mesh
new_elements_list = numpy.delete(original_elements_list, ordered_elements).tolist()
...
orderedNodes = dict.fromkeys(selected_element_nodes).keys()
...
# Find which elements lie on the surface of the mesh
for elem_num in new_elements_list:

    # Find out which nodes are associated with each element
    element_nodes = originalElements.NodesGet(elem_num,nodes_per_elem).tolist()
    
    # Find the coordinates of an element in the second mesh using its nodes
    for node_num in element_nodes:
        z = geometricField2.ParameterSetGetNodeDP(iron.FieldVariableTypes.U,iron.FieldParameterSetTypes.VALUES,1,1,node_num,3)

        # Set concentration to 0 and 1 at the opposing walls       
        if z == 0:
                       
            boundaryConditions.SetNode(dependentField,iron.FieldVariableTypes.U,1,1,node_num,1,iron.BoundaryConditionsTypes.FIXED,0.0) 

        elif z == length:

            boundaryConditions.SetNode(dependentField,iron.FieldVariableTypes.U,1,1,node_num,1,iron.BoundaryConditionsTypes.FIXED,1.0) 
...
fml.OutputAddFieldNoType(baseName+".phi", dataFormat, dependentField,
    iron.FieldVariableTypes.U, iron.FieldParameterSetTypes.VALUES)
fml.OutputWrite("ChangedBC.xml")
fml.Finalise()

# Export results
fields = iron.Fields()
fields.CreateRegion(region)
fields.NodesExport("ChangedBCResults","FORTRAN")
fields.ElementsExport("ChangedBCResults","FORTRAN")
fields.Finalise()
