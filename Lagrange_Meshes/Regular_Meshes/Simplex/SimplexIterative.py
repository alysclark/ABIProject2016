...
for time_idx in len(time):
    ...
    if time == 'Y':
        while round(time_residual,16) == steady_state_residual or time_residual == []:
    ...
    fml.OutputAddFieldNoType(baseName+".phi", dataFormat, dependentField,
        iron.FieldVariableTypes.U, iron.FieldParameterSetTypes.VALUES)
    fml.OutputWrite("SimplexExample.xml")
    fml.Finalise()

    # Export results
    fields = iron.Fields()
    fields.CreateRegion(region)
    fields.NodesExport("SimplexExampleResults","FORTRAN")
    fields.ElementsExport("SimplexExampleResults","FORTRAN")
    fields.Finalise()
