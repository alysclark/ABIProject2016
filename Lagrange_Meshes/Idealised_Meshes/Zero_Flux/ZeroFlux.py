...
def read_ansys_mesh(mesh_dir, filename, nodes_subset=[], elem_subset=[], debug=False):
    ...
    wall_node_array = numpy.empty((0,1),int)
    ...
    # Find which nodes are part of the wall
    for line_idx, line in enumerate(lines):
        if line.split(',')[0] == 'CMBLOCK' and line.split(',')[1] == 'WALLL':
            for node_line_idx in range(line_idx+2, num_lines+1):
                node_line = lines[node_line_idx]
                if node_line.split(',')[0] == 'CMBLOCK':
                    break
                else:
                    wall_node_row = node_line.split()
                    wall_node_row = scipy.array([int(node) for node in wall_node_row])
                    for wall_node in wall_node_row:
                        wall_node_array = numpy.append(wall_node_array,wall_node)
            break

    # Return node number and coordinate arrays, and element number and element node arrays
    return node_array, node_coordinates, element_array, element_nodes_array, inlet_node_array, outlet_node_array, wall_node_array

# Call the code which reads the ansys mesh
[node_array, node_coordinates, element_array, element_nodes_array, inlet_node_array, outlet_node_array, wall_node_array] = read_ansys_mesh(
    './', 'project1CoarseIO.in')
...
#    nodeDomain = decomposition.NodeDomainGet(int(node_num),1)
...
# Set zero flux at walls
for wall_node in wall_node_array:
    boundaryConditions.SetNode(dependentField,iron.FieldVariableTypes.DELUDELN,1,1,int(wall_node),1,iron.BoundaryConditionsTypes.FIXED,0.0) 
...
fml.OutputAddFieldNoType(baseName+".phi", dataFormat, dependentField,
    iron.FieldVariableTypes.U, iron.FieldParameterSetTypes.VALUES)
fml.OutputWrite("ZeroFlux.xml")
fml.Finalise()

# Export results
fields = iron.Fields()
fields.CreateRegion(region)
fields.NodesExport("ZeroFluxResults","FORTRAN")
fields.ElementsExport("ZeroFluxResults","FORTRAN")
fields.Finalise()
